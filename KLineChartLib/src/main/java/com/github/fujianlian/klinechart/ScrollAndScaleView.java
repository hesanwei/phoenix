 ...
 */
public abstract class ScrollAndScaleView extends FrameLayout implements
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if(System.currentTimeMillis() -mLastTouchTime <300 && Math.abs(event.getX() - x) < mTouchSlop){
                    isLongPress = true;
                    onLongPress(event);
                    invalidate();
                }else{
