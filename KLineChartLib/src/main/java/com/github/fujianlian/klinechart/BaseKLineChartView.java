...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private Paint mGridColumnPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mGridRowPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    //当前价格得画笔
    private Paint mCurrPriceTextBgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint mCurrPriceTextLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint mCurrPriceTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    boolean isCurrPriceUp = true;
    float currPrice = 52.81f;
    ...
    private void init() {
        ...
        mCurrPriceRect = new RectF();

        mGridRowPaint.setPathEffect(new DashPathEffect(new float[] {ViewUtil.Dp2Px(getContext(), 2f), ViewUtil.Dp2Px(getContext(), 2f)}, 0));

        mCurrPriceTextLinePaint.setPathEffect(new DashPathEffect(new float[] {ViewUtil.Dp2Px(getContext(), 2f), ViewUtil.Dp2Px(getContext(), 2f)}, 0));
        mCurrPriceTextPaint.setColor(Color.BLACK);
    }
    ...
    private void drawGird(Canvas canvas) {
        ...
        for (int i = 0; i <= mGridRows; i++) {
            canvas.drawLine(0, rowSpace * i + mMainRect.top, mWidth, rowSpace * i + mMainRect.top, mGridRowPaint);
        }
        //-----------------------下方子图------------------------
        if (mChildDraw != null) {
            canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom, mGridColumnPaint);
            canvas.drawLine(0, mChildRect.bottom, mWidth, mChildRect.bottom, mGridColumnPaint);
        } else {
            canvas.drawLine(0, mVolRect.bottom, mWidth, mVolRect.bottom, mGridColumnPaint);
        }
        //纵向的grid
        ...
        for (int i = 1; i < mGridColumns; i++) {
            canvas.drawLine(columnSpace * i, 0, columnSpace * i, mMainRect.bottom, mGridColumnPaint);
            canvas.drawLine(columnSpace * i, mMainRect.bottom, columnSpace * i, mVolRect.bottom, mGridColumnPaint);
            if (mChildDraw != null) {
                canvas.drawLine(columnSpace * i, mVolRect.bottom, columnSpace * i, mChildRect.bottom, mGridColumnPaint);
            }
        }
    }
    ...
    private void drawK(Canvas canvas) {
        ...
        //画当前价格虚线
        if (isCurrPriceUp) {
            mCurrPriceTextLinePaint.setColor(getResources().getColor(R.color.chart_red));
        }else  {
            mCurrPriceTextLinePaint.setColor(getResources().getColor(R.color.chart_green));
        }
        float currY = getMainY(currPrice);
        if (currY < mTopPadding) {
            currY =mTopPadding;
        }else if (currY > mMainRect.bottom) {
            currY = mMainRect.bottom;
        }
        canvas.drawLine(-mTranslateX, currY, -mTranslateX + mWidth / mScaleX, currY, mCurrPriceTextLinePaint);
    ...
    private void drawText(Canvas canvas) {
        ...
        //画当前价格得文字
        if (isCurrPriceUp) {
            mCurrPriceTextBgPaint.setColor(getResources().getColor(R.color.chart_red));
        }else  {
            mCurrPriceTextBgPaint.setColor(getResources().getColor(R.color.chart_green));
        }
        String currPriceText = formatValue(currPrice);
        float priceTextWidth = mTextPaint.measureText(currPriceText);
//        mCurrPriceTextPaint.getTextBounds(currPriceText, 0, );
        float vPadding = ViewUtil.Dp2Px(getContext(), 2);
        float hPadding = ViewUtil.Dp2Px(getContext(), 5);
        //y轴位置
        y = getMainY(currPrice);
        if (y < mTopPadding) {
            y =mTopPadding;
        }else if (y > mMainRect.bottom) {
            y = mMainRect.bottom;
        }
        mCurrPriceRect.right = mWidth-1;
        mCurrPriceRect.left = mCurrPriceRect.right-priceTextWidth - hPadding*2;
        mCurrPriceRect.top = y - textHeight/2f - vPadding;
        mCurrPriceRect.bottom = mCurrPriceRect.top + textHeight + vPadding*2;
        canvas.drawRect(mCurrPriceRect, mCurrPriceTextBgPaint);
        canvas.drawText(currPriceText, mCurrPriceRect.left+ hPadding+dp2px(1f), mCurrPriceRect.bottom -vPadding -dp2px(1.5f), mCurrPriceTextPaint);
    }
    ...
    public void setGridLineWidth(float width) {
        mGridColumnPaint.setStrokeWidth(width);
    }
    ...
    public void setGridLineColor(int color) {
        mGridColumnPaint.setColor(color);
    }

    /**
     * 设置表格线颜色
     */
    public void setGridRowLineColor(int color) {
        mGridRowPaint.setColor(color);
    }

    /**
     * 设置表格线宽度
     */
    public void setGridRowLineWidth(float width) {
        mGridRowPaint.setStrokeWidth(width);
    }
    ...
    public void setTextSize(float textSize) {
        ...
        mCurrPriceTextPaint.setTextSize(textSize);
    }
